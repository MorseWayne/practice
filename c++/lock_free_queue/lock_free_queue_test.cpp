...
// 测试多生产者单消费者场景
TEST_CASE("LockFreeQueue multiple producers single consumer") {
    LockFreeQueue<int> queue;
...
// 测试单生产者多消费者场景
TEST_CASE("LockFreeQueue single producer multiple consumers") {
    LockFreeQueue<int> queue;
...
// 测试多生产者多消费者场景
TEST_CASE("LockFreeQueue multiple producers multiple consumers") {
    LockFreeQueue<int> queue;
    const int producer_count = 4;
    const int consumer_count = 4;
...
// 测试队列性能和压力
TEST_CASE("LockFreeQueue performance and stress test") {
    LockFreeQueue<int> queue;
...
// 测试边界情况
TEST_CASE("LockFreeQueue edge cases") {
    LockFreeQueue<int> queue;
    
    SECTION("Test popping from empty queue") {
        REQUIRE(queue.pop() == false);
    }
    
    SECTION("Test front on empty queue") {
